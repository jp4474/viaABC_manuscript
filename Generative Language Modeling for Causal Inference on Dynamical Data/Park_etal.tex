...
\subsection*{VIA-ABC accurately extracts parameters embedded in the latent representations of noisy dynamical input}

We validate our approach using the Lotka-Volterra model~\citep{lotka1925elements}\citep{volterra1928variations}, which is a deterministic model of interaction between the populations of prey $x$ and predators $y$, expressed as a system of nonlinear differential equations:

\begin{equation}
\frac{dx}{dt} = a x - cxy, \qquad
\frac{dy}{dt} = b xy - dy.
\end{equation}
Here $a$ and  $b$ denote rates of population growth of the prey and their per capita loss due to predation. Similarly, parameters $c$ and $d$ represent the efficiency with which consumed prey are converted into predator offspring and loss of the predators.
Various studies have previously used the Lotka-Volterra equations as a model system to benchmark the parameter inference approach~\red{CITE}\citep{toni2009approximate}.
Typically these studies, fix the parameters $c=d=1$  and aim to infer \( \theta = (a, b)\) from prior distributions guided by either the density of the data given $\theta$ or a distance between weighted simulated and observed data.    %\(a, b \sim \mathrm{Uniform}(0, 10)\).
\blue{To methods likely
We followed the similar strategy and solved the Lotka-Volterra system using initial conditions \((x(0), y(0)) = (1, 0.5)\), which represent scaled population sizes.
We generated a test dataset by adding the Gaussian noise \(\epsilon_t \sim \mathcal{N}(0, 0.5^2)\) to  $x, y$ evaluated at eight distinct time points between \(t = 0\) and \(t = 15\), by setting \(\theta = (1, 1)\), similar to Toni and Khanh et al~\red{CITE}.
