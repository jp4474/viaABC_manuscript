...
We generated $N=50000$ training simulations of prey, predator dynamics~(shown as lines in~\textbf{Figure~\ref{fig:lv}A}), by sampling $\theta$ from the prior distributions using the \blue{latin hypercube sampler.}  
Each simulation produces a bivariate time series.
We selected eight time points between $t=0$ and $15$ to represent our \blue{training data
$u^{\mathrm{train}} = \cup_{n=1}^N\{x^n_1, y^n_1, \dots, x^n_8, y^n_8\}$}. %, where $n$ is an integer $\in [0, N]$. %, similar to Toni~\citep{toni2009approximate} \etal. 
We trained TSM-VAE on $u^{\mathrm{train}}$ such that \blue{$\sim12$\% }of each time series is masked from the encoder.
...
We then synthesized a test dataset (see methods for details) by evaluating $x, y$ at the same eight time points as the training samples, by setting \( \theta = (1, 1)\).
